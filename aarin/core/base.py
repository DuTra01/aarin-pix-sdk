from aarin.config import RequestOptions
from aarin.config import Config
...
class Base:
    ...
    def _patch(self, uri, data=None, params=None, request_options=None):
        if data is not None:
            data = JSONEncoder().encode(data)
        ...
        return self.__http_client.patch(url=self.__config.api_base_url + uri, data=data, params=params,
        headers=headers, timeout=request_options.connection_timeout,
        maxretries=request_options.max_retries)
